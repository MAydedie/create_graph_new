...
def _enumerate_branching_paths(
    ...
    *,
    ...
    for raw_start in start_methods or []:
        start = str(raw_start).strip()
        if not start or start in seen_starts:
            continue
    ...
    while queue and len(results) < max_paths:
        ...
        next_methods = sorted({
            str(item).strip()
            for item in (adjacency.get(current) or [])
            if str(item).strip() and str(item).strip() not in path
        })

        if not next_methods or len(path) >= max_depth:
        ...
        for next_method in next_methods:
            candidate_path = path + [next_method]
            if len(candidate_path) >= max_depth:
                candidate_tuple = tuple(candidate_path)
                if candidate_tuple not in seen_paths:
                    seen_paths.add(candidate_tuple)
...
def _merge_distinct_partition_paths(
    primary_paths: List[Dict[str, Any]],
    supplemental_paths: List[Dict[str, Any]],
    *,
    max_paths: int,
) -> List[Dict[str, Any]]:
    ...
    for item in list(primary_paths or []) + list(supplemental_paths or []):
        if not isinstance(item, dict):
            continue
        normalized_path = tuple(
            str(part).strip()
            for part in (item.get('function_chain') or item.get('path') or [])
            if str(part).strip()
        )
        if not normalized_path or normalized_path in seen_keys:
            continue
        seen_keys.add(normalized_path)
        merged.append(item)
...
def _extract_lightweight_partition_paths(analysis: Dict[str, Any], partition_methods: Optional[List[str]] = None, max_paths: int = 4) -> List[Dict[str, Any]]:
    ...
    candidate_paths = _enumerate_branching_paths(start_methods, adjacency, max_paths=max_paths, max_depth=4)
    if not candidate_paths:
        candidate_paths = [[method] for method in start_methods[:max_paths] if method]
...
def _extract_structural_partition_paths(analysis: Dict[str, Any], max_paths: int = 4) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    if valid_candidates:
        ...
        if len(selected_candidates) < max_paths and fallback_candidates:
            supplemental_selected, supplemental_deferred = _select_representative_paths(
                fallback_candidates,
                max_paths - len(selected_candidates),
            )
            selected_candidates = _merge_distinct_partition_paths(
                selected_candidates,
                supplemental_selected,
                max_paths=max_paths,
            )
            deferred_candidates.extend(supplemental_deferred)
            total_candidates += len(fallback_candidates)
            if supplemental_selected:
                selection_policy = 'structural_paths_map_supplemented'
...
def _get_partition_path_payload(analysis: Dict[str, Any], partition_methods: Optional[List[str]] = None) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    path_info = analysis.get('path_analysis_info') or {}
    structural_max_paths = int(path_info.get('max_paths_limit') or max(len(path_analyses), 4) or 4)
    structural_paths, structural_info = _extract_structural_partition_paths(analysis, max_paths=structural_max_paths)
    ...
    if path_analyses:
        ...
        merged_paths = _merge_distinct_partition_paths(
            path_analyses,
            structural_paths,
            max_paths=structural_max_paths,
        )
        supplemented_count = max(0, len(merged_paths) - len(path_analyses))
        if supplemented_count > 0:
            merged_info['selection_policy'] = 'deep_analysis_supplemented'
            merged_info['selected_count'] = len(merged_paths)
            merged_info['supplemented_from_structural_count'] = supplemented_count
            merged_info['completion_status'] = 'complete_with_structural_supplement'
            merged_info['user_message'] = '当前结果已自动补齐更多结构路径，避免功能分区只显示单条链路。'
            if structural_info.get('deferred_path_summaries') and not merged_info.get('deferred_path_summaries'):
                merged_info['deferred_path_summaries'] = structural_info.get('deferred_path_summaries')
            return merged_paths, merged_info
    ...
    lightweight_paths = _extract_lightweight_partition_paths(analysis, partition_methods=partition_methods, max_paths=structural_max_paths)
...
def _build_entrypoint_enriched_path_payload(
    ...
    *,
    ...
    entry_methods = [
        str(entry.get('method_signature') or '').strip()
        for entry in entry_points
        if isinstance(entry, dict) and str(entry.get('method_signature') or '').strip()
    ]
    candidate_paths = _enumerate_branching_paths(entry_methods, adjacency, max_paths=max_paths, max_depth=4)
    if not candidate_paths:
        candidate_paths = [[method_sig] for method_sig in entry_methods[:max_paths] if method_sig]

    enriched: List[Dict[str, Any]] = []
    for index, path in enumerate(candidate_paths):
        method_sig = path[0] if path else ''
        if not method_sig:
            continue
...
def _build_process_payloads_from_graph_contract(project_path: str, graph_contract: Dict[str, Any]) -> List[Dict[str, Any]]:
    raw_nodes = graph_contract.get('nodes')
    nodes: List[Dict[str, Any]] = [item for item in raw_nodes if isinstance(item, dict)] if isinstance(raw_nodes, list) else []
    raw_relationships = graph_contract.get('relationships')
    relationships: List[Dict[str, Any]] = [item for item in raw_relationships if isinstance(item, dict)] if isinstance(raw_relationships, list) else []
    ...
    for node in nodes:
        if str(node.get('label') or '') != 'Process':
            continue
        process_id = str(node.get('id') or '').strip()
        if not process_id:
            continue
        properties_raw = node.get('properties')
        properties: Dict[str, Any] = properties_raw if isinstance(properties_raw, dict) else {}
        communities_raw = properties.get('communities')
        communities = [str(item).strip() for item in communities_raw if str(item).strip()] if isinstance(communities_raw, list) else []
        processes[process_id] = {
            'process_id': process_id,
            'entry': str(properties.get('entryPointId') or properties.get('name') or process_id),
            'entry_node_id': str(properties.get('entryPointId') or ''),
            'label': str(properties.get('heuristicLabel') or properties.get('name') or process_id),
            'processType': str(properties.get('processType') or 'intra_community'),
            'communities': communities,
            'stepCount': int(properties.get('stepCount') or 0),
            'steps': [],
            'project_path': project_path,
        }

    sorted_relationships = sorted(
        [rel for rel in relationships if str(rel.get('type') or '') == 'STEP_IN_PROCESS'],
        key=lambda rel: int(rel.get('step') or 10**9),
    )

    for rel in sorted_relationships:
        process_id = str(rel.get('targetId') or '').strip()
        step_id = str(rel.get('sourceId') or '').strip()
        if not process_id or not step_id or process_id not in processes:
            continue
        step_node = node_by_id.get(step_id) or {}
        step_props_raw = step_node.get('properties')
        step_props: Dict[str, Any] = step_props_raw if isinstance(step_props_raw, dict) else {}
        step_number = rel.get('step')
        process_steps_raw = processes[process_id].get('steps')
        process_steps: List[Dict[str, Any]] = process_steps_raw if isinstance(process_steps_raw, list) else []
        try:
            normalized_step_number = int(step_number) if step_number is not None else len(process_steps) + 1
        except (TypeError, ValueError):
            normalized_step_number = len(process_steps) + 1
        process_steps.append({
            'id': step_id,
            'name': str(step_props.get('name') or step_id),
            'filePath': str(step_props.get('filePath') or ''),
            'stepNumber': normalized_step_number,
        })
        processes[process_id]['steps'] = process_steps

    result: List[Dict[str, Any]] = []
    for process in processes.values():
        steps_raw = process.get('steps')
        steps: List[Dict[str, Any]] = steps_raw if isinstance(steps_raw, list) else []
        if int(process.get('stepCount') or 0) <= 0:
            process['stepCount'] = len(steps)
        result.append(process)

    result.sort(key=lambda item: int(item.get('stepCount') or 0), reverse=True)
    return result


def _resolve_process_compat_payload(normalized_project_path: str) -> Optional[Dict[str, Any]]:
    payload = _resolve_process_shadow_payload(normalized_project_path)
    if isinstance(payload, dict):
        return payload

    graph_data = _resolve_graph_data_for_project(normalized_project_path, allow_global_fallback=True)
    if not isinstance(graph_data, dict):
        return None

    graph_contract = _convert_graph_data_to_gn_contract(normalized_project_path, graph_data)
    processes = _build_process_payloads_from_graph_contract(normalized_project_path, graph_contract)
    if not processes:
        return None
    ...
    return {
        'project_path': normalized_project_path,
        'processes': processes,
        'source': 'graph_contract_fallback',
    }
...
def api_get_processes_compat():
    ...
    requested_repo = request.args.get('repo')
    project_path = requested_project_path or _infer_repo_project_path(requested_repo) or str(PROJECT_ROOT)
    normalized_project_path = os.path.normpath(project_path)
    payload = _resolve_process_compat_payload(normalized_project_path)
    if payload is None:
        return jsonify({'error': '未找到 Phase3 影子结果'}), 404
