...
export const DropZone = ({ onFileSelect, onGitClone, onServerConnect }: DropZoneProps) => {
  ...
  const handleLocalPathAnalyze = async () => {
    ...
    setPathProgress({
      phase: 'starting',
      progress: 1,
      message: 'Starting unified analysis session...',
    });
    ...
    try {
      ...
      for (let attempt = 0; attempt < 900; attempt += 1) {
        ...
        setPathProgress({
          phase: status.phase,
          progress: Math.max(1, Math.min(99, status.progress || 0)),
        ...
        if (status.bootstrapReady || status.status === 'completed') {
          completed = true;
          break;
        }
      }
      ...
      if (!completed) {
        throw new Error('Analysis timed out. Please wait and retry.');
      }

      setPathProgress({
        phase: 'bootstrap_ready',
        progress: 99,
        message: 'Loading analyzed graph into GitNexus...',
      });
        ...
        (phase, downloaded, total) => {
          if (phase === 'downloading') {
            ...
            setPathProgress({
              ...
              message: total
                ? `Downloading graph ${percent}%`
                : `Downloading graph ${formatBytes(downloaded)}`,
            });
            return;
          }
      ...
      const message = err instanceof Error ? err.message : 'Failed to analyze local path';
      ...
      <div className="relative w-full max-w-lg">
        ...
        {activeTab === 'path' && (
          <div className="p-8 bg-surface border border-border-default rounded-3xl">
            ...
            <p className="text-sm text-text-secondary text-center mb-6">
              Enter an absolute path and run create_graph unified analysis on backend.
            </p>
              ...
              <div className="flex gap-2">
                ...
                >
                  {isPathAnalyzing ? (
                    <>
                      <Loader2 className="w-5 h-5 animate-spin" />
                      {pathProgress.message || 'Analyzing...'}
                    </>
                  ) : (
                    <>
                      Analyze Path
            ...
            {isPathAnalyzing && (
              <div className="mt-4">
                ...
                <p className="mt-2 text-xs text-text-muted text-center">
                  {pathProgress.phase} • {Math.max(1, Math.min(100, pathProgress.progress || 1))}%
                </p>
              </div>
            )}
